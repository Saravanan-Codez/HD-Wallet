...
const DashBoard = ({ seedPhrase, password, onLogout }) => {
  ...
  useEffect(() => {
    ...
    const fetchAllBalances = async () => {
      for (const wallet of wallets) {
        try {
          const bal = await getSOLBalance(wallet.publicKeyBase58);
  ...
  return (
    <div className="min-h-screen w-full flex flex-col bg-neutral-950 text-white font-sans selection:bg-neutral-800">
      ...
      {/* --- Header / Navigation --- */}
      <header className="w-full border-b border-neutral-800 bg-neutral-900/50 backdrop-blur-xl sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-6 lg:px-8 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-medium tracking-tight text-white hidden sm:block">HD Wallet</h1>
          </div>

          <div className="flex items-center gap-5">
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-neutral-800/50 border border-neutral-700/50">
            ...
            <button 
              onClick={onLogout}
              className="px-4 py-2 rounded-full text-sm font-medium text-neutral-400 border border-neutral-800 hover:bg-neutral-800 hover:text-white transition-colors"
            >
              Sign out
            </button>
          </div>
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-6 lg:px-8 py-8 md:py-12">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
          ...
          <div className="lg:col-span-8 flex flex-col gap-8">
            ...
            {/* Balance Hero Card */}
            <div className="bg-neutral-900/40 border border-neutral-800 rounded-[2.5rem] p-8 md:p-12 relative overflow-hidden flex flex-col justify-between md:min-h-90">
              {/* Decorative Glow */}
              <div className="absolute top-[-50%] right-[-10%] w-[80%] h-full rounded-full bg-white/5 blur-[100px] pointer-events-none"></div>

              <div className="relative z-10 w-full flex justify-between items-start mb-12">
                <div>
                  <p className="text-base font-medium text-neutral-400 mb-2">Total Balance</p>
                  <div className="text-5xl md:text-7xl font-light tracking-tight text-white flex items-baseline gap-3">
                    {loadingBalance ? (
                      <span className="w-48 h-16 bg-neutral-800/80 rounded-2xl animate-pulse inline-block"></span>
                    ) : (
                       <>{balance} <span className="text-3xl md:text-4xl text-neutral-500 font-normal">SOL</span></>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="relative z-10 flex flex-col sm:flex-row gap-4">
                <button
                  ...
                  className="flex-1 py-4 px-6 rounded-2xl bg-white text-black font-semibold text-lg hover:scale-[1.02] hover:shadow-[0_8px_30px_rgba(255,255,255,0.15)] transition-all duration-200 active:scale-95 flex justify-center items-center gap-2"
                >
                ...
                <button
                  onClick={() => alert("Receive view not implemented in this demo snippet")}
                  className="flex-1 py-4 px-6 rounded-2xl border border-neutral-700 bg-neutral-900/80 text-white font-medium text-lg hover:bg-neutral-800 transition-all duration-200 active:scale-95 flex justify-center items-center gap-2"
                >
                  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div className="bg-neutral-900/20 border border-neutral-800/80 rounded-4xl p-8 md:p-10 mb-8">
          ...
          <div className="lg:col-span-4 flex flex-col gap-8">
            ...
            <div className="bg-neutral-900/30 border border-neutral-800 rounded-4xl p-8">
